...
const getContainerCls = () => {
  return {
    container: true,
    pointer: !scope.isLocked
  };
}
</script>
<template>
  <div class="buttons">
    <VPButton :text="scope.isLocked ? 'Get Started' : 'Stop Drawing'" @click="scope.isLocked = !scope.isLocked" />
  </div>
  <div ref="container" :class="getContainerCls()"></div>
</template>
<style scoped>
.buttons {
  margin-bottom: 16px;
